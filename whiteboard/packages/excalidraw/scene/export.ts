...
import { arrayToMap, distance, getFontString, sceneCoordsToViewportCoords, toBrandedType } from "../utils";
...
export const exportToCanvasWithin = async (
  elements: readonly NonDeletedExcalidrawElement[],
  appState: AppState,
  files: BinaryFiles,
  {
  ...
  const width = Math.min(window.devicePixelRatio * appState.width, 1500);
  const height = Math.min(window.devicePixelRatio * appState.width, 1500);
  ...
  renderStaticScene({
    ...
    appState: {
      ...
      frameRendering,
      viewBackgroundColor: exportBackground ? viewBackgroundColor : null,
      scrollX: -(window.devicePixelRatio * appState.width / 2) + (width / 2),
      scrollY: -(window.devicePixelRatio * appState.height / 2) + (height / 2),
      zoom: defaultAppState.zoom,
      shouldCacheIgnoreZoom: false,
      theme: appState.exportWithDarkMode ? THEME.DARK : THEME.LIGHT,
    },
